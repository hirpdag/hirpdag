...
they just cannot be roots. Each root type gets a field named `root_` plus the
...
let bytes = hirpdag_serialize(&HirpdagArchiveRoots {
    root_Expr: vec![e1, e2],
    root_Variables: vec![vars],
})?;

let out = hirpdag_deserialize(&bytes)?;
let e1_again: &Expr = &out.root_Expr[0];
```
...
the `root_<Type>` field name in JSON). Nodes
