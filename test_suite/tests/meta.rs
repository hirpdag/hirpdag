...
// generated code computes it and `hirpdag_get_meta()` reads it back. The
// `HirpdagMeta` arithmetic is unit tested in hirpdag::base::meta; this checks
// how the generated `hirpdag_compute_meta` folds a node's fields: numbers,
// strings, `Option`, `Vec`, enum payloads and child nodes.

use hirpdag::base::HirpdagMeta;
use hirpdag::*;
...
fn count_and_height(meta: &HirpdagMeta) -> (u32, u16) {
...
fn a_node_without_children_counts_itself() {
    assert_eq!(count_and_height(leaf(1).hirpdag_get_meta()), (1, 1));
}

#[test]
fn a_chain_counts_every_node() {
    let chain = Tree::new(3, vec![Tree::new(2, vec![leaf(1)])]);
    assert_eq!(count_and_height(chain.hirpdag_get_meta()), (3, 3));
}
...
fn siblings_add_counts_and_take_the_taller_height() {
    ...
    assert_eq!(count_and_height(parent.hirpdag_get_meta()), (4, 3));
}
...
fn a_shared_child_is_counted_once_per_reference() {
    ...
    assert_eq!(count_and_height(top.hirpdag_get_meta()), (6, 3));
}

#[test]
fn leaf_fields_contribute_nothing() {
    let holder = Holder::new(Payload::Plain(7), None);
    assert_eq!(count_and_height(holder.hirpdag_get_meta()), (1, 1));
}
...
fn enum_payloads_and_options_pass_their_contents_through() {
    ...
    assert_eq!(count_and_height(holder.hirpdag_get_meta()), (4, 3));
}
