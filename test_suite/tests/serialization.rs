...
fn binary_round_trip_pointer_equal() {
    ...
    let roots = HirpdagArchiveRoots {
        roots_Item: vec![parent.clone()],
        ..Default::default()
    };
    let bytes = hirpdag_serialize(&roots).unwrap();
    let out = hirpdag_deserialize(&bytes).unwrap();

    assert_eq!(out.roots_Item.len(), 1);
    assert!(out.roots_Node.is_empty());
    assert!(out.roots_Pair.is_empty());
    // Hirpdag equality is pointer equality: the deserialized root re-interned
    // to the exact same node.
    assert_eq!(out.roots_Item[0], parent);
    assert_eq!(out.roots_Item[0].deps[0], out.roots_Item[0].deps[2]);
}
...
fn enum_payload_round_trip() {
    ...
    let roots = HirpdagArchiveRoots {
        roots_Node: vec![sum.clone()],
        ..Default::default()
    };
    let bytes = hirpdag_serialize(&roots).unwrap();
    let out = hirpdag_deserialize(&bytes).unwrap();
    let sum2 = &out.roots_Node[0];
    assert_eq!(*sum2, sum);
    match &sum2.kind {
        Kind::Sum(items) => {
...
fn sharing_preserved_fibonacci() {
    ...
    let out = hirpdag_deserialize_json(&text).unwrap();
    assert_eq!(out.roots_Node[0], curr);
}
...
fn multiple_roots_share_nodes() {
    ...
    let roots = HirpdagArchiveRoots {
        roots_Item: vec![r1.clone(), r2.clone()],
        ..Default::default()
    };
    let text = hirpdag_serialize_json(&roots).unwrap();
    let value: hirpdag::serde_json::Value = hirpdag::serde_json::from_str(&text).unwrap();
    ...
    let out = hirpdag_deserialize_json(&text).unwrap();
    assert_eq!(out.roots_Item.len(), 2);
    assert_eq!(out.roots_Item[0], r1);
    assert_eq!(out.roots_Item[1], r2);
    // The shared subgraph is the same node in both deserialized roots.
    assert_eq!(out.roots_Item[0].deps[0], out.roots_Item[1].deps[0]);
}
...
fn mixed_type_roots() {
    ...
    let roots = HirpdagArchiveRoots {
        roots_Pair: vec![p.clone()],
        roots_Item: vec![i.clone()],
        ..Default::default()
    };
    let text = hirpdag_serialize_json(&roots).unwrap();
    let value: hirpdag::serde_json::Value = hirpdag::serde_json::from_str(&text).unwrap();
    ...
    let out = hirpdag_deserialize_json(&text).unwrap();
    assert_eq!(out.roots_Pair[0], p);
    assert_eq!(out.roots_Item[0], i);
    assert_eq!(out.roots_Pair[0].tag, Label::new("mixed_tag".to_string()));
    // The Item root and the Pair's children are the same interned node.
    assert_eq!(out.roots_Pair[0].left, out.roots_Item[0]);
}
...
fn deserialize_twice_merges() {
    ...
    let roots = HirpdagArchiveRoots {
        roots_Item: vec![root.clone()],
        ..Default::default()
    };
    let bytes = hirpdag_serialize(&roots).unwrap();

    let r1 = hirpdag_deserialize(&bytes).unwrap().roots_Item[0].clone();
    let r2 = hirpdag_deserialize(&bytes).unwrap().roots_Item[0].clone();
...
fn truncated_input_rejected() {
    let roots = HirpdagArchiveRoots {
        roots_Item: vec![Item::new("trunc_item".to_string(), vec![])],
        ..Default::default()
    };
    let bytes = hirpdag_serialize(&roots).unwrap();
    ...
    let threads: Vec<_> = (0..8)
        .map(|t| {
            std::thread::spawn(move || {
                ...
                let bytes = hirpdag_serialize(&roots).unwrap();
                let out = hirpdag_deserialize(&bytes).unwrap();
                assert_eq!(out.roots_Item[0], root);
                assert_eq!(out.roots_Item[0].deps[0], leaf);
            })
        })
        .collect();
...
fn node_type_mismatch_rejected() {
    ...
    let text =
        r#"{"version":1,"nodes":[{"Item":{"name":"x","deps":[]}}],"roots":{"roots_Node":[0]}}"#;
    let err = hirpdag_deserialize_json(text).unwrap_err();
    assert_eq!(
        err,
...
fn unknown_roots_field_rejected() {
    ...
    // `roots_<Type>` carry the old snake-cased names, and without
...
fn schema_mismatch_rejected() {
    let roots = HirpdagArchiveRoots {
        roots_Item: vec![Item::new("schema_item".to_string(), vec![])],
        ..Default::default()
    };
    let bytes = hirpdag_serialize(&roots).unwrap();
...
fn schema_match_accepted_across_modules_with_same_shape() {
    ...
    let roots = HirpdagArchiveRoots {
        roots_Item: vec![Item::new("schema_same".to_string(), vec![])],
        ..Default::default()
    };
    let bytes = hirpdag_serialize(&roots).unwrap();
    let out = hirpdag_deserialize(&bytes).unwrap();
