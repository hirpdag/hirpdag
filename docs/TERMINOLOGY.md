...
### Rewriter
A user-defined struct that implements the generated `HirpdagRewriter` trait.  It has one method per `#[hirpdag]` type (`rewrite_Foo`, `rewrite_Bar`, …) and is the single entry point for tree transformations.  Each method takes a recursion driver `rec: &impl HirpdagRewriter`; recurse into children through `rec` (e.g. `x.default_rewrite(rec)`) rather than through `self`, so a memoizing wrapper can intercept every node.

Generated by: `#[hirpdag_module]`

### Recursion driver (`rec`)
The rewriter that child nodes are rewritten through, passed as the first argument to every `rewrite_*` method.  At a traversal's entry it is the rewriter itself, so a plain rewriter recurses through `self` as before.  For `HirpdagRewriteMemoized` it is the memoizing wrapper, so recursion re-enters the cache at every node.  Separating the driver from `self` (the object holding the per-type rule) is what lets memoization cover the whole traversal instead of only the outermost node.

### `default_rewrite`
The default traversal provided for every `#[hirpdag]` type.  It rewrites each child field through the driver, then reconstructs the node with the new children (returning the original if nothing changed).  Override individual `rewrite_*` methods to intercept specific node types.  Public, so rewriters may be defined outside the hirpdag module (any fields they read must then be `pub`).

### Memoization
`HirpdagRewriteMemoized<Rewriter>` wraps any `HirpdagRewriter` and caches each node's rewrite result in a per-type `RefCell<HashMap<Ref, Ref>>` (interior mutability — the `rewrite_*` methods take `&self`).  The wrapper is the recursion driver for the whole traversal, so on a DAG with shared subtrees each unique node is rewritten once (`O(unique nodes)`), not once per path.  The map key is a `Ref`, whose `Hash`/`Eq` are O(1) (creation ID / pointer).  See `docs/adr/0003-interior-mutability.md`.
