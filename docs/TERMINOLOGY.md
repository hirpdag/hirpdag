...
### Memoization
`HirpdagRewriteMemoized<Rewriter>` wraps any `HirpdagRewriter` and memoizes the result of each `rewrite_*` call.  It owns a `HirpdagRewriteCache` — a per-struct-type `Mutex<HashMap<Node, Node>>`, so the cache is interior-mutable and thread-safe — held behind an `Arc`.  While a rewrite runs it installs that cache as the current thread's cache; the generated recursion (`hirpdag_rewrite`) consults it on every node, so recursion re-enters the memoizer regardless of whether the current rewriter is the wrapper or the inner rewriter.  A node reached by multiple paths in a hash-consed DAG is therefore rewritten once and served from the cache on subsequent encounters — within a traversal and across repeated `rewrite` calls (the cache lives as long as the wrapper).
