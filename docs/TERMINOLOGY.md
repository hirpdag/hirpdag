...
| Driver | Traversal |
| --- | --- |
| `HirpdagRewriteDirect<'_, Rewriter>` | Applies the rules on every path; a node reached N ways is rewritten N times.  `rewriter.rewrite(&x)` is shorthand for it. |
| `HirpdagRewriteMemoized<Rewriter>` | Applies the rules once per unique node, serving repeats from a cache. |
...
### Memoization
`HirpdagRewriteMemoized<Rewriter>` is the caching driver: it wraps any `HirpdagRewriter` and remembers the result of every `rewrite_*` call in a per-type `HashMap` from input node to output node.  Because nodes are hash-consed, those keys hash and compare by interned identity (creation ID / pointer) — an `O(1)` lookup that replaces re-traversing a shared subtree, turning a walk that is exponential in the sharing into one rule invocation per unique node.  The caches are filled through interior mutability (`RefCell`), so rewriting takes `&self`; a memoizer is consequently single-threaded, and holds its cached nodes alive until it is dropped or `clear_caches()` is called.  Cached results assume the rules are a pure function of the node.
...
### `#[hirpdag_module]`
The attribute macro.  Applied to an inline module (`mod name { ... }`), it expands every struct and enum in the module marked `#[hirpdag]`, passes other items through unchanged, and generates the module-level code (the `HirpdagRewriter` trait with one `rewrite_*` method per type, the `HirpdagRewriteDriver` trait with the `HirpdagRewriteDirect` and `HirpdagRewriteMemoized` drivers, and the serialization machinery).  The hash-consing configuration is given as attribute arguments: a named `preset = "..."` or the explicit type overrides listed below.  The generated code refers to the hirpdag crate by absolute paths, so the module needs no imports beyond what the user's own code uses.  Must be an outer attribute on an inline module; Rust does not accept the inner form (`#![hirpdag_module]`, rust-lang/rust#54726).
