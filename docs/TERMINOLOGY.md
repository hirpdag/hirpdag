...
### Memoization
A rewriter opts into memoization by owning a `HirpdagRewriteCache` and returning `Some(&cache)` from `HirpdagRewriter::memoized_rewrite_cache` (the default is `None`, i.e. no memoization).  `HirpdagRewriteCache` is a per-struct-type `Mutex<HashMap<Node, Node>>`, so it is interior-mutable and thread-safe.  Because rewriters recurse through `self`, the generated `hirpdag_rewrite` consults the rewriter's own cache on every node — a node reached by multiple paths in a hash-consed DAG is rewritten once and served from the cache on subsequent encounters, both within a traversal and across repeated `rewrite` calls (the cache lives as long as the rewriter).
