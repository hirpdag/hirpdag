...
To associate *mutable* state with a node (annotations, analysis results, scratch state), keep it in a side-table keyed by the node reference (e.g. `HashMap<Node, _>`) rather than inside the node.  A hash-consed `HirpdagRef` is a stable, `O(1)`-hashable/ordered key.  See `book/src/ch04-00-techniques.md`.
...
### Rewriter
A user-defined struct that implements the generated `HirpdagRewriter` trait.  It has one method per `#[hirpdag]` type (`rewrite_Foo`, `rewrite_Bar`, …) and is the single entry point for tree transformations.  A `rewrite_*` method recurses by calling `x.default_rewrite(self)` (or `self.rewrite(&child)`) — always through `self`.
...
### Memoization
Memoization is **on by default**.  Every `HirpdagRewriter` also supplies a `HirpdagMemoize<HirpdagRewriteCache>` (a supertrait).  The common case is `#[derive(HirpdagMemoize)]` on a rewriter that owns a `HirpdagRewriteCache` field: the derive finds the field and hands back the cache, so there is no accessor to write.  To turn memoization off, a rewriter implements `HirpdagMemoize` by hand to return `None` (then it needs no cache field).  `HirpdagRewriteCache` is a per-struct-type `Mutex<HashMap<Node, Node>>`, so it is interior-mutable and thread-safe.  Because rewriters recurse through `self`, the generated `hirpdag_rewrite` consults the rewriter's own cache on every node — a node reached by multiple paths in a hash-consed DAG is rewritten once and served from the cache on subsequent encounters, both within a traversal and across repeated `rewrite` calls (the cache lives as long as the rewriter).
...
### `#[hirpdag_module]`
The attribute macro.  Applied to an inline module (`mod name { ... }`), it expands every struct and enum in the module marked `#[hirpdag]`, passes other items through unchanged, and generates the module-level code (the `HirpdagRewriter` trait with one `rewrite_*` method per type, the `HirpdagRewriteCache` used for memoization, and the serialization machinery).  The hash-consing configuration is given as attribute arguments: a named `preset = "..."` or the explicit type overrides listed below.  The generated code refers to the hirpdag crate by absolute paths, so the module needs no imports beyond what the user's own code uses.  Must be an outer attribute on an inline module; Rust does not accept the inner form (`#![hirpdag_module]`, rust-lang/rust#54726).
