...
### Memoization
Every `HirpdagRewriter` must implement `memoized_rewrite_cache` (there is no default) — deciding whether it memoizes.  To memoize, a rewriter owns a `HirpdagMemoizeCache` and returns `Some(&self.cache)`; to skip memoization it returns `None` and keeps no cache field.  `HirpdagMemoizeCache` is a general-purpose, per-struct-type `Mutex<HashMap<Node, Node>>`, so it is interior-mutable and thread-safe.  Because rewriters recurse through `self`, the generated `hirpdag_rewrite` consults the rewriter's own cache on every node — a node reached by multiple paths in a hash-consed DAG is rewritten once and served from the cache on subsequent encounters, both within a traversal and across repeated `rewrite` calls (the cache lives as long as the rewriter).
...
### `#[hirpdag_module]`
The attribute macro.  Applied to an inline module (`mod name { ... }`), it expands every struct and enum in the module marked `#[hirpdag]`, passes other items through unchanged, and generates the module-level code (the `HirpdagRewriter` trait with one `rewrite_*` method per type, the `HirpdagMemoizeCache` used for memoization, and the serialization machinery).  The hash-consing configuration is given as attribute arguments: a named `preset = "..."` or the explicit type overrides listed below.  The generated code refers to the hirpdag crate by absolute paths, so the module needs no imports beyond what the user's own code uses.  Must be an outer attribute on an inline module; Rust does not accept the inner form (`#![hirpdag_module]`, rust-lang/rust#54726).
