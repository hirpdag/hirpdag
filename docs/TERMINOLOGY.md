...
### Rewriter
A user-defined struct that implements the generated `HirpdagRewriter` trait.  It has one method per `#[hirpdag]` type (`rewrite_Foo`, `rewrite_Bar`, …) and is the single entry point for tree transformations.  Each `rewrite_*` method takes a recursor `rec: &R` alongside the node; recurse through `rec` (rather than `self`) so that when the rewriter is wrapped in `HirpdagRewriteMemoized` the recursion re-enters the cache.

Generated by: `#[hirpdag_module]`

### `default_rewrite`
The default traversal provided for every `#[hirpdag]` type.  It rewrites each child field through the recursor, then reconstructs the node with the new children (returning the original if nothing changed).  Override individual `rewrite_*` methods to intercept specific node types, passing the method's `rec` argument to `default_rewrite` for the fields you don't handle specially.  Public, so rewriters may be defined outside the hirpdag module (any fields they read must then be `pub`).

### Memoization
`HirpdagRewriteMemoized<Rewriter>` wraps any `HirpdagRewriter` and caches the result of each `rewrite_*` call in a per-type `RefCell<HashMap<Node, Node>>`.  It threads itself through the recursion as the recursor, so a node reached by multiple paths in a hash-consed DAG is rewritten once and served from the cache on subsequent encounters (within a traversal and across repeated `rewrite` calls).
