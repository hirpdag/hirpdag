...
### Memoization
Memoization is **on by default**.  A rewriter owns a `HirpdagRewriteCache` and returns it from `HirpdagRewriter::rewrite_cache` (`&self.cache`); the default `memoized_rewrite_cache` returns `Some(self.rewrite_cache())`.  To turn memoization off, a rewriter overrides `memoized_rewrite_cache` to return `None` (then it needs no cache).  `HirpdagRewriteCache` is a per-struct-type `Mutex<HashMap<Node, Node>>`, so it is interior-mutable and thread-safe.  Because rewriters recurse through `self`, the generated `hirpdag_rewrite` consults the rewriter's own cache on every node — a node reached by multiple paths in a hash-consed DAG is rewritten once and served from the cache on subsequent encounters, both within a traversal and across repeated `rewrite` calls (the cache lives as long as the rewriter).
