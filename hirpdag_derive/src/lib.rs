...
/// The hirpdag attribute macro.
///
/// Applied to an inline module, it expands every struct and enum in the
/// module marked with an inert `#[hirpdag]` attribute into a hash-consed
/// data type, passes other items through unchanged, and appends the
/// module-level machinery (the HirpdagRewriter trait, memoized rewriting,
/// and serialization) at the end of the module.
///
/// The hash-consing configuration is given as attribute arguments: either a
/// named `preset = "..."` or the explicit `reference_type`,
/// `reference_weak_type`, `table_type`, `tableshared_type`, and
/// `build_tableshared_type` strings. With no arguments a sensible default
...
/// ```
///
/// The generated code refers to the hirpdag crate by absolute paths, so no
/// imports are required inside the module. Everything is generated by this
/// single macro invocation, so there is no ordering or state shared between
/// separate attribute expansions; see
/// docs/adr/0002-module-attribute-macro.md. The attribute must be an outer
/// attribute: Rust does not accept the inner form (`#![hirpdag_module]`,
