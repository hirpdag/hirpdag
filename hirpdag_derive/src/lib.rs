                ...
                D: hirpdag::serde::Deserializer<'de>,
            {
                ...
                HIRPDAG_DE_SESSION.with(|cell| {
                    ...
                    match &nodes[index as usize] {
                        HirpdagAnyRef::#hirpdag_ref_name(r) => Ok(r.clone()),
...
pub fn hirpdag_end(
    ...
    let struct_type_names: Vec<String> = guard
        .iter()
        .filter(|entry| entry.is_struct)
        .map(|entry| entry.name.clone())
        .collect();
    let serialization_items = get_serialization_items(&struct_type_names);
...
/// Generates the module-level serialization machinery: the archive node and
/// any-ref enums, the collect context, the thread-local (de)serialization
/// sessions, and the public entry points.
fn get_serialization_items(struct_type_names: &[String]) -> proc_macro2::TokenStream {
    if struct_type_names.is_empty() {
        // No hashconsed types in this module; nothing to serialize.
        return proc_macro2::TokenStream::new();
    }

    let mut archive_variants = proc_macro2::TokenStream::new();
    let mut anyref_variants = proc_macro2::TokenStream::new();
    let mut intern_arms = proc_macro2::TokenStream::new();
    let mut anyref_collect_arms = proc_macro2::TokenStream::new();
    let mut anyref_conversions = proc_macro2::TokenStream::new();
    ...
    for name in struct_type_names {
        ...
        intern_arms.extend(quote! {
            HirpdagArchiveNode::#ref_name(data) => HirpdagAnyRef::#ref_name(#ref_name(
                hirpdag::base::HirpdagStruct::hirpdag_hashcons(data),
            )),
        });
        anyref_collect_arms.extend(quote! {
            HirpdagAnyRef::#ref_name(r) => hirpdag::base::HirpdagCollect::hirpdag_collect(r, ctx),
        });
        anyref_conversions.extend(quote! {
            impl From<#ref_name> for HirpdagAnyRef {
                fn from(x: #ref_name) -> Self {
                    HirpdagAnyRef::#ref_name(x)
                }
            }

            impl std::convert::TryFrom<HirpdagAnyRef> for #ref_name {
                type Error = hirpdag::base::HirpdagSerializeError;
                fn try_from(x: HirpdagAnyRef) -> Result<Self, Self::Error> {
                    match x {
                        HirpdagAnyRef::#ref_name(r) => Ok(r),
                        #[allow(unreachable_patterns)]
                        _ => Err(hirpdag::base::HirpdagSerializeError::TypeMismatch {
                            expected: #name,
                        }),
                    }
                }
    ...
    quote! {
        ...
        /// A reference to a node of any hirpdag type in this module.
        /// Used for serialization roots and deserialization results.
        #[derive(Clone, Debug, Hash, PartialEq, Eq)]
        #[derive(hirpdag::serde::Serialize, hirpdag::serde::Deserialize)]
        #[serde(crate = "hirpdag::serde")]
        #[allow(dead_code)]
        pub enum HirpdagAnyRef {
            #anyref_variants
        }

        impl hirpdag::base::HirpdagCollect<HirpdagCollectCtx> for HirpdagAnyRef {
            fn hirpdag_collect(&self, ctx: &mut HirpdagCollectCtx) {
                match self {
                    #anyref_collect_arms
                }
        ...
        std::thread_local! {
            ...
            static HIRPDAG_DE_SESSION: std::cell::RefCell<Option<Vec<HirpdagAnyRef>>> =
                std::cell::RefCell::new(None);
        }
        ...
        impl HirpdagDeSessionGuard {
            fn open() -> Result<Self, hirpdag::base::HirpdagSerializeError> {
                HIRPDAG_DE_SESSION.with(|cell| {
                    let mut borrow = cell.borrow_mut();
                    if borrow.is_some() {
                        return Err(hirpdag::base::HirpdagSerializeError::SessionActive);
        ...
        fn hirpdag_collect_archive(
            ...
            for root in roots {
                hirpdag::base::HirpdagCollect::hirpdag_collect(root, &mut ctx);
            }
        ...
        /// through the hashcons table, and returns the roots in the order
        /// they were serialized.
        #[allow(dead_code)]
        pub fn hirpdag_deserialize(
            bytes: &[u8],
        ) -> Result<Vec<HirpdagAnyRef>, hirpdag::base::HirpdagSerializeError> {
